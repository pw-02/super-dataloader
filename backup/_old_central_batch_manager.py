...
class PrefetchService:
    def __init__(self, prefetch_lambda_name: str):
    ...
    def avg_execution_time(self):
        if self.prefetch_execution_times.count > 0:
            return self.prefetch_execution_times.avg
        else:
           return self.prefetch_lambda.avg_execution_time()
...
class DLTJob:
    ...
    def next_training_step_batch(self, prfetch_service:PrefetchService, dataset: Dataset, cache_address: str) -> Optional[Batch]:
        self.total_steps += 1
        if self.batches_in_cycle: 
            return self.batches_in_cycle.pop(0)
        else: 
            #cycele is empty so start a new cycle
            cycle_duration = prfetch_service.avg_execution_time()
            time_counter = 0
            prefetch_counter = 0
            ...
            optimal_prefetch_conncurrency = find_optimal_prefetch_conncurrency(cycle_duration, self.training_step_times_on_hit.avg)
            ...
            for batch_id, batch in list(self.future_batches.items()):  
                ...
                if  time_counter < cycle_duration:
                    if batch.is_cached:
                        time_counter += self.training_step_times_on_hit.avg
                        self.batches_in_cycle.append(batch)
                    else: #potentally chage this to if batch is not caching in progress to avoid redundant prefetches
                        time_counter += self.training_step_times_on_miss.avg
                        self.batches_in_cycle.append(batch)
                    continue
            ...
            if len(prefetch_list) > 0:
                self.prefetch_batches(prefetch_list)
            if len(keep_alive_list) > 0:
                self.prefetch_batches(keep_alive_list)
            return self.batches_in_cycle.pop(0)
...
class CentralBatchManager:
    ...
    def get_next_batch(self, job_id: str, previous_step_training_time:float, previous_step_is_cache_hit:bool, previous_step_gpu_time:float) -> Optional[Batch]:
        with self.lock:    
            ...
            batch = job.next_training_step_batch(self.prefetch_lambda, dataset=self.dataset, cache_address=self.cache_address)
            return batch
