...
class PrefetchService:
    def __init__(self, prefetch_lambda_name: str, 
        ...
        self.prefetch_stop_event:threading.Event = threading.Event().set()  # Event to signal preftch stopping #set() to stop prefetching
    ...
    def _prefetching_process(self):
        ...
        while not self.prefetch_stop_event.is_set():
            try:
                prefetch_list:Set[Tuple[Batch,Dict[str, any]]] = set()
                prefetch_cycle_duration = self.lambda_execution_times.avg + self.prefetch_delay
                ...
                for job in self.jobs.values():
                    ...
                    avg_time_on_hit = job.training_step_times_on_hit.avg
                    ...
                    for batch in job.future_batches.values():
                        ...
                        time_counter += avg_time_on_hit if batch.is_cached else avg_time_on_miss
                        # If the prefetch cycle duration is exceeded, queue up the batch for prefetching
                        if time_counter > prefetch_cycle_duration:
                            prefetch_counter += 1
                            if not batch.is_cached and not batch.caching_in_progress:
                                batch.set_caching_in_progress(True)
                                payload = {
                                    'bucket_name': job.dataset.bucket_name,
                                    'batch_id': batch.batch_id,
                                    'batch_samples': job.dataset.get_samples(batch.indicies),
                                    'cache_address': self.cache_address,
                                    'task': 'prefetch',
                                }
                                prefetch_list.add((batch, payload))
                        # If the optimal prefetch concurrency is reached, break the loop
                        if prefetch_counter >= optimal_prefetch_lookahead:
                            break
