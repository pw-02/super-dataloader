...
# Simulated time intervals (e.g., seconds, training steps)
time_intervals = np.arange(0, 100, 1)  # From time 0 to 100
# Simulating the number of items in cache over time
# Assume the cache starts empty and gradually fills up, then fluctuates as old items get evicted
cache_items = np.minimum(time_intervals**1.2, 5000)  # Growth curve with an upper cap at 5000 items
cache_items += np.random.normal(scale=100, size=len(time_intervals))  # Add some fluctuation
...
ax1.plot(time_intervals, cache_items, color='blue', linewidth=2, linestyle='-', label='Number of Items in Cache')

# Set labels and title
ax1.set_xlabel('Time (hours)', fontsize=12)
ax1.set_ylabel('Number of Items in Cache', fontsize=11) #show it increasing over time
