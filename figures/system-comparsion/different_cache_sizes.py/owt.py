...
workload_data[workload_name] =  {
    ...
    # "CacheHit" : { "LitData": {'80': 25, '60': 50, '40': 75, '20': 100},
...
for workload in workload_data:
    ...
    for i, sh in enumerate(workload_cost[r'$\bf{DisDP}$'].values()):
        ...
        if i == 0:
            ax2.text(x[i] + barwidth/2 +0.1, sh + 14, f'x{savings:.1f} \n savings', ha='center', va='top', fontsize=11, color='black', fontweight='bold')
        else:
            ax2.text(x[i] + barwidth/2 +0.05, sh + 11.5, f'x{savings:.1f}', ha='center', va='top', fontsize=11, color='black', fontweight='bold')
