...
def simulate_training(num_traning_steps, num_jobs, ttl, cost_per_request):
    """Simulate training with multiple jobs and a cache, incrementing the number of steps continuously."""
    cache = Cache(ttl)
    ...
    for start_batch in range(0, num_traning_steps, num_jobs):
        batch_set = list(range(start_batch, min(start_batch + num_jobs, num_traning_steps)))
    ...
    print(f"Cache Cost for this batch set: ${cost:.2f}\n")

# Configuration
NUM_TRAINING_STEPS = 25  # Total number of batches
